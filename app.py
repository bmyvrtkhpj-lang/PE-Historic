...
# --- ULTRA-PREMIUM INSTITUTIONAL CSS ---
st.markdown("""
    <style>
    /* Deep Stealth Background - Less blue, more pure dark */
    .stApp {
    ...
    /* Top Control Bar Styling (Make it look like a unified nav bar) */
    .block-container {
    ...
    /* Sleek Metric Cards - Glassmorphism effect */
    div[data-testid="metric-container"] {
        background: linear-gradient(145deg, #11151c, #161b24);
    ...
    /* Metric values - Neon green accent */
    ...
    /* The RUN button - High contrast accent */
    ...
    /* Popover/Expander clean up */
    .st-expander, .stPopover {
        border-color: #1f2937 !important;
        border-radius: 4px !important;
    }
    
    /* Reduce gap between top bar and chart */
    hr {
        margin-top: 0.5rem;
...
def render_stock_chart(ticker, merged):
    ...
    # High-Vis Institutional Markers (Cyan & Pink)
    fig.add_trace(go.Scatter(x=buys.index, y=buys["price"], mode="markers", name="Heavy Buy",
                              marker=dict(color="#06b6d4", size=10, symbol="triangle-up", 
                                          line=dict(color="#083344", width=1))))
    fig.add_trace(go.Scatter(x=sells.index, y=sells["price"], mode="markers", name="Heavy Sell",
                              marker=dict(color="#ec4899", size=10, symbol="triangle-down", 
                                          line=dict(color="#500724", width=1))))
                                          
    # PE Percentile (Subtle Orange Dashed)
    fig.add_trace(go.Scatter(x=merged.index, y=merged["pe_percentile"] * 100, name="PE %Rank",
                              yaxis="y2", line=dict(width=1, dash="dot", color="#f59e0b")))
                              
    fig.update_layout(
        template="plotly_dark",
        paper_bgcolor='rgba(0,0,0,0)',
        plot_bgcolor='rgba(0,0,0,0)',
        title=dict(text=f"SIGNAL ANALYSIS: {ticker}", font=dict(size=16, color="#f8fafc", family="monospace")),
        height=550,
        yaxis=dict(title="Price (INR)", showgrid=True, gridcolor="#1e293b", zeroline=False),
...
def render_ablation_table(ablation_results, side):
    ...
    for key, label in labels.items():
        ...
        rows.append({
            ...
            "Return (63d)": f"{row['avg_signal_return']:.2%}" if pd.notnull(row['avg_signal_return']) else "-",
            "Win Rate": f"{row['win_rate']:.2%}" if pd.notnull(row['win_rate']) else "-",
            "p-value": f"{row['p_value']:.4f}" if pd.notnull(row['p_value']) else "-",
        })
    if rows:
...
def main():
    st.markdown("<h3 style='color: #94a3b8; font-family: monospace; letter-spacing: -1px;'>SYSTEMATIC QUANT FRAMEWORK</h3>", unsafe_allow_html=True)
    
    # --- SLEEK TOP CONTROL BAR ---
    # Using vertical_alignment="bottom" makes the run button align perfectly with inputs
    c1, c2, c3, c4 = st.columns([2.5, 1.5, 1, 1], vertical_alignment="bottom")
    
    with c1:
        TICKER_MAPPING = get_all_nse_stocks()
        company_name = st.selectbox("Universe Selection", options=list(TICKER_MAPPING.keys()), index=None, placeholder="Search Ticker / Company...")
        ticker = TICKER_MAPPING.get(company_name) if company_name else ""
        
    with c2:
        xlsx_file = st.file_uploader("Fundamental Data", type=["xlsx"])
        
    with c3:
        # All messy sliders hidden inside a sleek popover menu
        with st.popover("⚙️ Strategy Params", use_container_width=True):
            st.markdown("**Valuation Bounds**")
            cheap_pctile = st.slider("Cheap PE %", 0.05, 0.50, 0.20, 0.05)
            expensive_pctile = st.slider("Expensive PE %", 0.50, 0.95, 0.80, 0.05)
            
            st.markdown("**Technical Triggers**")
    ...
    if not ticker or xlsx_file is None:
        st.info("Awaiting inputs. Select a universe ticker and upload fundamental .xlsx to execute the backtest.")
        return

    if run_btn:
        with st.spinner(f"Processing data arrays for {ticker}..."):
        ...
        d1.metric("Trading Days (Total)", len(merged))
        d2.metric("Valid PE Array", int(merged["pe"].notna().sum()))
        d3.metric("Heavy Buy Signals", int(merged['heavy_buying'].sum()))
        d4.metric("Heavy Sell Signals", int(merged['heavy_selling'].sum()))

        st.markdown("<br>", unsafe_allow_html=True)
        
        # Edge-to-edge Charting
        ...
        c1, c2 = st.columns(2)
        with c1:
            st.markdown("<span style='color:#06b6d4; font-family:monospace;'>[ BUY ZONE ] FORWARD RETURNS</span>", unsafe_allow_html=True)
            st.dataframe(result["results"]["buy_signal_eval"], use_container_width=True, hide_index=True)
        with c2:
            st.markdown("<span style='color:#ec4899; font-family:monospace;'>[ SELL ZONE ] FORWARD RETURNS</span>", unsafe_allow_html=True)
        ...
        a1, a2 = st.columns(2)
        with a1:
            st.markdown("<span style='color:#94a3b8; font-family:monospace;'>BUY ALPHA ATTRIBUTION (63d)</span>", unsafe_allow_html=True)
            render_ablation_table(result["ablation_results"], "buy")
        with a2:
            st.markdown("<span style='color:#94a3b8; font-family:monospace;'>SELL ALPHA ATTRIBUTION (63d)</span>", unsafe_allow_html=True)
